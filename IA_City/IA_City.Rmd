...
```{r}
png('./county_vehicle_plot_street.png', width = 1920, height = 1080)
...
iac_city_map = get_map(getbb("Iowa City"))
ggmap(iac_city_map) + ggplot(.data=topstreets, aes(x=X, y=Y, label=LITERAL)) + 
    geom_point(data=crash_data, aes(x=X, y=Y), show.legend=FALSE) + 
    geom_point(aes(color=count), size=10) + geom_text_repel(aes(color=count), size=8, nudge_y=-0.1, nudge_x=-0.2, label.padding=2, box.padding=2, show.legend=FALSE) +
